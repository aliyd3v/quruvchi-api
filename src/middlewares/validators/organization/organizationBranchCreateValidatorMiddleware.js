...
const organizationBranchCreateDto = z
  .object(
    {
      parent_id: z.number({ message: "organization_id_must_be_number" }).refine((value) => !Number.isNaN(value) && value > 0, { message: "organization_id_must_be_positive_integer" }),
...
exports.organizationBranchCreateValidatorMiddleware = (req, _res, next) => {
  const parsed = organizationBranchCreateDto.safeDecode(req.body);
  if (parsed.error) {
