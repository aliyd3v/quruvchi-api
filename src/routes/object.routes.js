...
objectRouter
  ...
  .get("/trash", checkRoleMiddleware(Roles.SUPERADMIN), objectController.deletedObjects)
  .patch("/trash/:id", checkRoleMiddleware(Roles.SUPERADMIN), objectController.restoreDeleted)
  .delete("/trash/:id", checkRoleMiddleware(Roles.SUPERADMIN), objectController.absoluteDelete)
  .get("/:id", checkPermissionMiddleware(Permissions.object_crud), objectController.getOne)
  .get("/:id/workers", checkRoleMiddleware(Roles.SUPERADMIN, Roles.ACCOUNTANT, Roles.ADMIN), objectController.getObjectWorkers)
